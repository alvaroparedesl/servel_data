...
```{r, include=FALSE}
# rmarkdown::render('G:/GIT/servel_data/website/childs/summary_.Rmd', output_file = 'G:/GIT/servel_data/website/childs/summary.html')
knitr::opts_knit$set(root.dir = 'G:/GIT/servel_data/')
...
```{r, fig.align='center'}
waffle(round(p2_), rows = 10, colors = c("#006D2C", "#a1d99b"))
```
...
```{r}
DT::datatable(p_, options = list(dom = 't', pageLength = 500, columnDefs = list(list(className = 'dt-center', targets = "_all"))), rownames=F) |>
...
DT::datatable(p_p, options = list(dom = 't', pageLength = 500, columnDefs = list(list(className = 'dt-center', targets = "_all"))), rownames=F) |>
  DT::formatPercentage(columns = 2:ncol(p_), mark=".", dec.mark=",", digits=2)
```
...
DT::datatable(regp, options = list(dom = 't', pageLength = 500, columnDefs = list(list(className = 'dt-center', targets = "_all"))), 
...
com_per[, body:=sprintf('%s<br>%s', header, hoverlabels[CUT_COM])]
...
com__ <- merge(com_, com__[, c('CUT_COM')])
com__ <-  as(com_, 'Spatial')
pal <- colorNumeric('RdBu', domain=0:100, reverse=T)
labels <- sprintf(
  "<b>Comuna</b>: %s<br><b>Participación</b>: %s",
  com__$nom_com, com__$labl
...
m |> addPolygons(
  ...
  addLegend(pal = pal, values = 0:100, opacity = 0.9, title = '% <i>Izquierda</i>', position = "bottomright")
```
